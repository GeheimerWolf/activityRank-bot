...
export const role = subcommand({
  data: {
    ...
    options: [
      ...
      {
        name: 'change',
        description: 'The amount of XP to give to each member with the role. This option may be negative.',
        min_value: -1_000_000,
        max_value: 1_000_000,
      ...
      {
        name: 'use-beta',
        description: 'Enables the beta method of giving bonus to roles. Warning: will not send levelUpMessages',
  ...
  async execute({ interaction, client }) {
    ...
    const clean = () => {
      if (currentJobs.delete(interaction.guild.id)) {
        client.logger.warn(`role bonus job removed during sweep from guild ${interaction.guild.id}`);
      } else {
    ...
    if (interaction.options.getBoolean('use-beta')) return await betaSystem(interaction, role, change);
    else return await oldSystem(interaction, role, change);
  },
});

async function oldSystem(interaction: ChatInputCommandInteraction<'cached'>, role: Role, changeAmount: number) {
  await interaction.deferReply();

  interaction.client.logger.debug(`Starting old role give; fetching ${interaction.guild.memberCount} members`);
...
async function betaSystem(interaction: ChatInputCommandInteraction<'cached'>, role: Role, changeAmount: number) {
  ...
  interaction.client.logger.debug(`New role give to guild with member count ${interaction.guild.memberCount}`);
  ...
  for (const member of members) {
    await statFlushCache.directlyAddBonus(member, interaction.guild, interaction.client, changeAmount);
...
function progressBar(index: number, max: number, len = 40) {
  ...
  return `${'■'.repeat(progress)}${'□'.repeat(len - progress)} ${Math.round(fraction * 1_000) / 10}%`;
}
