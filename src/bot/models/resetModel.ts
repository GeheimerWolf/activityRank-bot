...
abstract class ResetJob {
  ...
  /**
   * Updates the interaction message with the current status of the job, likely generated by {@link getStatusContent}.
  ...
  public async run(options?: { bufferTime?: number }): Promise<boolean> {
    ...
    try {
      ...
      // keeps the queue locked and unusable for bufferTime ms
      if (options?.bufferTime) await sleep(options.bufferTime);
    } finally {
      // unlock queue. Requires `finally` block to avoid deadlocking.
      RESET_QUEUE.shift();
    }
